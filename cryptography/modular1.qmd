...
Before wrapping up today's activities, we'll take a brief look at *modular arithmetic*.
...
For example, $5 \bmod 3 = 2$, because the remainder when dividing $5$ by $3$ is $2$.
But also, $8 \bmod 3 = 2$; so $5$ and $8$ are equivalent, or *congruent*, modulo $3$.
...
1. $(a^m) \equiv (p^m) \bmod n$, for all integers $m$.
...
But if we notice that $33 \equiv 1 \bmod{32}$, then we can see that
