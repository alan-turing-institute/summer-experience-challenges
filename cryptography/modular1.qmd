...
This example highlights one common misconception about modular arithmetic.
It is often said that $5 \equiv 2 \bmod 3$, because 2 is the remainder when dividing 5 by 3.
However, this is not entirely accurate: it is more correct to say that $5 \equiv 2 \bmod 3$ because 5 and 2 have the same remainder when divided by 3.
In other words, the numbers on either side of the $\equiv$ sign are not the *remainders* themselves, but the numbers that have the same remainder when divided by $n$.
...
To sum up: **$a$ and $b$ are congruent modulo $n$ if $a$ and $b$ have the same remainder when divided by $n$.**
...
To do this proof, we need to use one key fact: if $a \equiv p \bmod n$, that means that there is some integer $c$ for which $a = p + cn$.
...
For example, what is the remainder when $33^{594}$ is divided by $32$?
...
> **Challenge:** Calculate the remainder when $7^{175}$ is divided by $16$.
>
